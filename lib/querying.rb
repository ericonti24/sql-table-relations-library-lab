...
def select_character_names_and_number_of_books_they_are_in
  "SELECT Characters.name, COUNT(Character_books.character_id) FROM Character_books LEFT OUTER JOIN Characters ON Characters.id = Character_books.character_id GROUP BY(Characters.id) ORDER BY COUNT(Character_books.character_id) DESC, Characters.name ASC;"
end
